...
## 三、可用主题（7 套，禁用摸鱼绿 / 留白禅意）

> **以下为缓存，以 `references/themes/theme-index.md` 为唯一权威来源。**
> 每次排版前 Read 一次 theme-index.md；若内容与下表不一致，以 theme-index.md 为准并同步更新本表。
> 本 skill **只允许使用下面 7 套**。主题库里另有 摸鱼绿 `moyu-green`、留白禅意 `zen-whitespace` 两套，**已被本 skill 禁用**，不读取、不选用、不向用户展示。
...
2. 依据文章题材，从第三节【可用主题（7 套）】里自动选定一套：
   - 题材明显契合某套的"适用场景"→ 选契合套
...
| 用户要的风格 7 套可用主题都不覆盖 | 走「自定义主题生成」流程（`references/themes/theme-generator.md`），生成并登记进 theme-index.md 后再回到 4.0 |
