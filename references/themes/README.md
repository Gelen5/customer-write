...
## 三、可用主题（6 套，禁用摸鱼绿 / 留白禅意）

> **以下为缓存，以 `references/themes/theme-index.md` 为唯一权威来源。**
> 每次排版前 Read 一次 theme-index.md；若内容与下表不一致，以 theme-index.md 为准并同步更新本表。
> 本 skill **只允许使用下面 6 套**。主题库里另有 摸鱼绿 `moyu-green`、留白禅意 `zen-whitespace` 两套，**已被本 skill 禁用**，不读取、不选用、不向用户展示。橄榄手记 `olive-journal`、墨蓝批注 `ink-note`、琥珀速读 `amber-skim`、黛青复盘 `timeline-review` 已删除下架，不得选用。
...
2. 依据文章题材，从第三节【可用主题（6 套）】里自动选定一套：
   - 题材明显契合某套的"适用场景"→ 选契合套
...
**禁止**：`<style>` / `<script>` / `<div>`、`class` / `id` 属性、`position` 任何值、`float`、`@media` / `@keyframes`、`display:grid`、CSS 变量、外部字体、SVG SMIL 动画、伪元素、`calc()`、`background:` 简写、任何渐变（`linear-gradient` 等——公众号会过滤，底色丢失/被替换成灰底）。
**必须**：样式全内联；背景色一律 `background-color` 实色，白底顶层块显式写 `background-color:#FFFFFF`；**所有文字节点用 `<span leaf="">文字</span>` 包裹**（漏包裹会导致粘贴后样式整片丢失，是最高频致命错）；`<img>` 加 `display:block`。
...
| 用户要的风格 6 套可用主题都不覆盖 | 走「自定义主题生成」流程（`references/themes/theme-generator.md`），生成并登记进 theme-index.md 后再回到 4.0 |
