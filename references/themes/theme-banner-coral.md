...
**骨架铁律**：火热标题最前；版块用珊瑚粗字 + 渐变短线；班型卡两列；报名用珊瑚 CTA；正文每段 1~3 处珊瑚下划线。**不使用「后台回复关键词/扫码看公告」引导模块（已按用户要求删除）；不使用 END 收尾标语模块（已按用户要求删除），文章以最后一个内容组件自然结束。**
...
| 招考公告引流 | 火热标题2 + 正文4 + 号召6 | 按钮10 |

所有类型共用：火热标题 2 + 版块标题 3。文末不加 END 收尾标语（已删除）。
